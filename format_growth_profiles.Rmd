...
This document outlines the steps used to format the growth profile data.
...
# these ge and strain tax xlsx files were created by duplicating the original files in refac_med/data_tidy on 1/17/21
...
dOD %>%
  ggplot(aes(x = dOD)) +
  geom_density() +
  labs(title = 'Distribution of dODs') +
  theme_classic()
```
...
# curves and boxplot 
cowplot::plot_grid(a, b, ncol = 1, rel_heights = c(1, 2))
```
